...
if (typeof(Vue)!=='undefined') {
    ...
    Vue.component('pagination', {
        ...
        template:'<ul class="uk-pagination">' +
                ...
                '<li v-if="page.has_previous"><a v-on:click = "gotoPage( page.page_index - 1, user_id)" href="#0"><i class="uk-icon-angle-double-left"></i></a></li>' +
                '<li class="uk-active"><span v-text="page.page_index"></span></li>' +
                '<li v-if="! page.has_next" class="uk-disabled"><span><i class="uk-icon-angle-double-right"></i></span></li>' +
                '<li v-if="page.has_next"><a v-on:click = "gotoPage( page.page_index + 1, user_id)" href="#0"><i class="uk-icon-angle-double-right"></i></a></li>' +
            '</ul>',
        methods:
        {
            gotoPage:function(page_index, user_id){
                dicts = {}
                dicts.page = page_index;
                if (typeof(user_id) !== 'undefined')
                {
                    dicts.user_id = user_id;
                }
                getJSON('/blog/view',dicts, function (err, results) {
                    if (err) {
                        return fatal(err);
                    }
                    initVM(results);
